...
// Traefik is internal/pkg/values/traefik.yaml.tmpl.
type Traefik struct {
	Replicas int
	...
	// constants from internal/pkg/hetzner — see the template for what a mismatch does.
	NodePortHTTP  int
	NodePortHTTPS int
}

// Cilium is internal/pkg/values/cilium.yaml.tmpl.
type Cilium struct {
...
// CCM is internal/pkg/values/hcloud-ccm.yaml.tmpl.
type CCM struct {
...
// ArgoCD is internal/pkg/values/argo-cd.yaml.tmpl.
type ArgoCD struct {
...
var probes = map[string]any{
	...
	"cilium": Cilium{
		PodCIDR: "198.51.100.0/24", APIHost: "localhost",
