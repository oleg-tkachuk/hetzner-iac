...
//
// These spellings are the Hetzner API's own: internal/pkg/hetzner/servertypes compares
...
// dataplane at all.
//
// A named constant rather than a literal in machineconfig.go because internal/pkg/cni
// has to be able to check against it — two halves of one decision, in one
// place, which is the same reason internal/pkg/platform exists.
